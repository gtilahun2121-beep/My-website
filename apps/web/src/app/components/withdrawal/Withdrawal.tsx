...
export const Withdrawal: React.FC<WithdrawalProps> = ({ balance, onSuccess, onCancel }) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg max-w-md w-full overflow-hidden">
      {/* Header */}
      <div className="bg-gradient-to-r from-[#314fa0] to-[#2a4183] text-white p-4 flex justify-between items-center">
        <div>
            ...
            <div key={label} className="flex items-center gap-2">
              <div
                className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                  done ? 'bg-emerald-500 text-white' : active ? 'bg-[#314fa0] text-white' : 'bg-gray-200 text-gray-500'
                }`}
              >
                {done ? '✓' : i + 1}
              </div>
              <span className={`text-xs font-medium ${active ? 'text-[#314fa0]' : 'text-gray-500'}`}>{label}</span>
      ...
      <div className="p-6">
          ...
          {step === 'pin-entry' && (
            <>
              <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-xl p-4 mb-6 text-center border border-emerald-100">
              ...
              <label className="block font-semibold text-gray-900 mb-2">Amount to Withdraw (ETB)</label>
              <input
                type="number"
                inputMode="decimal"
                value={amount}
                ...
                className="w-full p-4 rounded-lg border border-gray-300 focus:border-[#314fa0] focus:outline-none mb-4"
              />

              <label className="block font-semibold text-gray-900 mb-2">Enter your registered password / PIN</label>
              <div className="relative mb-4">
                <input
                  ...
                  placeholder=""
                  className="w-full text-center text-2xl tracking-[0.5em] p-4 pr-14 rounded-lg border border-gray-300 focus:border-[#314fa0] focus:outline-none"
                />
                <button
                  type="button"
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  ...
                  <button
                    ...
                    onClick={() => setShowPin((v) => !v)}
                    className={`relative inline-flex items-center h-6 w-11 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#314fa0] focus:ring-offset-2 ${
                      showPin ? 'bg-[#314fa0]' : 'bg-gray-300'
                    }`}
                  >
                    <span
              ...
              <button
                ...
                className="w-full bg-[#314fa0] text-white px-4 py-3 rounded-lg font-semibold hover:bg-[#2a4183] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Verify PIN & Continue
              </button>
            </>
          ...
          {step === 'method-selection' && (
            <>
              <p className="font-semibold text-gray-900 mb-4">Select Withdrawal Method</p>
          ...
          {step === 'account-entry' && (
            <>
              ...
              <input
                ...
                className="w-full p-3 rounded-lg border border-gray-300 focus:border-emerald-500 focus:outline-none mb-4"
              />

              <button
                onClick={handleLookupAccountName}
                className="w-full bg-gray-100 text-gray-700 px-4 py-3 rounded-lg font-semibold hover:bg-gray-200 transition-colors mb-4"
              >
              ...
              <button
                ...
                className="w-full bg-emerald-600 text-white px-4 py-3 rounded-lg font-semibold hover:bg-emerald-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
              ...
              <button
                ...
                className="w-full mt-3 border border-gray-300 text-gray-700 px-4 py-3 rounded-lg font-semibold hover:bg-gray-50 transition-colors"
              >
                Back to Methods
              </button>
            </>
          ...
          {step === 'confirmation' && (
            <div className="text-center py-8">
              ...
              <p className="text-lg font-semibold text-gray-900 mb-2">Withdrawal Successful!</p>
