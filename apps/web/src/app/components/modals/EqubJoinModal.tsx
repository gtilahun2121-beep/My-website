...
export default function EqubJoinModal({
  ...
  const colorMap = {
    blue: { bg: 'bg-brand-50', border: 'border-brand-200', text: 'text-brand-600', button: 'bg-[#0066ff] hover:bg-[#0047b3]' },
    amber: { bg: 'bg-brand-50', border: 'border-brand-200', text: 'text-brand-600', button: 'bg-[#0066ff] hover:bg-[#0047b3]' },
    green: { bg: 'bg-brand-50', border: 'border-brand-200', text: 'text-brand-600', button: 'bg-[#0066ff] hover:bg-[#0047b3]' },
  };
        ...
        >
          ...
          >
            ...
            <div className={`${colors.bg} border-b ${colors.border} px-3 py-2 flex items-center justify-between`}>
              <h3 className="text-sm font-bold text-[#0066ff]">{info.title}</h3>
              <button
                onClick={handleClose}
              ...
              <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    <div className="space-y-1.5 mb-3">
                      <div className={`p-2 ${colors.bg} border ${colors.border} rounded`}>
                        ...
                        <p className="text-sm font-bold text-[#0066ff]">{info.contribution}</p>
                      </div>

                      <div className={`p-2 ${colors.bg} border ${colors.border} rounded`}>
                        <p className="text-xs text-gray-600 font-semibold">Duration</p>
                        <p className="text-sm font-bold text-[#0066ff]">{info.duration}</p>
                      </div>

                      <div className={`p-2 ${colors.bg} border ${colors.border} rounded`}>
                        <p className="text-xs text-gray-600 font-semibold">Payout</p>
                        <p className="text-sm font-bold text-[#0066ff]">{info.potSize}</p>
                      </div>
                    </div>

                    {/* Terms */}
                    <div className="bg-gray-50 p-2 rounded border border-gray-200 mb-3">
                      <p className="text-xs font-semibold text-[#0066ff] mb-1">Agree to:</p>
                      <ul className="space-y-0.5 text-xs text-[#0066ff]">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={handleClose}
                        className="flex-1 px-2 py-1.5 border border-gray-300 rounded font-bold text-xs text-[#0066ff] hover:bg-gray-50"
                      >
                        Cancel
                      </button>
                      <button
                        onClick={handleJoin}
                        className={`flex-1 px-2 py-1.5 ${colors.button} text-[#0066ff] font-bold text-xs rounded`}
                      >
                        Join Now
                      </button>
                    </div>
                ...
                {step === 'joining' && (
                  ...
                  >
                    ...
                    <p className="text-xs font-bold text-[#0066ff]">Processing...</p>
                  </motion.div>
                )}
                ...
                {step === 'success' && (
                  ...
                  >
                    ...
                    <h4 className="text-sm font-bold text-[#0066ff] text-center mb-1">
                      Success!
                    </h4>

                    <div className={`w-full p-2 ${colors.bg} border ${colors.border} rounded mb-2`}>
                      <p className="text-xs font-bold text-[#0066ff]">Next: Make first contribution</p>
                    </div>
                    ...
                    <button
                      ...
                      className={`w-full px-2 py-1.5 ${colors.button} text-[#0066ff] font-bold text-xs rounded`}
                    >
                      Go to Dashboard
                    </button>
                  </motion.div>
