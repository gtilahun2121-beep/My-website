  ...
  return (
    <>
      ...
      >
        ...
        <motion.div
          variants={itemVariants}
          className="bg-[#f5f3f0] border-2 border-[#0066ff] rounded-lg p-4"
        >
          <p className="font-black text-[#0066ff] mb-3">💵 Filter by Income Level</p>
            ...
            <motion.button
              ...
              className={`py-2 px-3 rounded-lg font-black text-xs transition-all ${
                selectedIncome === 'all'
                  ...
                  : 'bg-white border-2 border-[#0066ff] text-white'
              }`}
            >
              All
            </motion.button>
        ...
        <motion.div
          ...
          className="space-y-3"
        >
          <h3 className="font-black text-[#0066ff] text-lg">🏢 Available Equb Groups ({displayedEqubs.length})</h3>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex gap-2 flex-wrap text-xs">
                        ...
                        <span className={`px-2 py-1 rounded font-bold ${
                          ...
                          equb.incomeLevel === 'medium' ? 'bg-brand-100 text-brand-700' :
        ...
        >
          <div className="bg-[#0066ff] text-white rounded-lg p-4 text-center">
          ...
          >
            <div className="flex justify-between items-start mb-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h2 className="text-2xl font-black text-[#0066ff]">{selectedEqub.name}</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
              <div className="bg-[#0066ff]/10 rounded-lg p-4">
                ...
                <p className="text-xl sm:text-2xl font-black text-[#0066ff] break-all">ETB {selectedEqub.monthlyPayment.toLocaleString()}</p>
              </div>
              <div className="bg-[#0066ff]/20 rounded-lg p-4">
                <p className="text-xs text-gray-600 mb-1">Expected Return</p>
                <p className="text-xl sm:text-2xl font-black text-[#0066ff] break-all">ETB {selectedEqub.expectedReturn.toLocaleString()}</p>
              </div>
              <div className="bg-brand-100 rounded-lg p-4">
                <p className="text-xs text-gray-600 mb-1">Active Members</p>
                <p className="text-xl sm:text-2xl font-black text-brand-700 break-all">{selectedEqub.members.toLocaleString()}</p>
              </div>
              <div className={`rounded-lg p-4 ${
                selectedEqub.incomeLevel === 'low' ? 'bg-yellow-100' :
                selectedEqub.incomeLevel === 'medium' ? 'bg-brand-100' :
                'bg-brand-100'
              }`}>
                <p className="text-xs text-gray-600 mb-1">Income Level</p>
                <p className={`text-2xl font-black ${
                  selectedEqub.incomeLevel === 'low' ? 'text-yellow-700' :
                  selectedEqub.incomeLevel === 'medium' ? 'text-brand-700' :
                  'text-brand-700'
                }`}>
            ...
            <div className="bg-[#f5f3f0] border-2 border-[#0066ff] rounded-lg p-4 mb-6">
              <h3 className="font-black text-[#0066ff] mb-2">⏰ Payout Frequency</h3>
              <p className="text-sm font-bold text-[#0066ff]">{selectedEqub.payoutFrequency}</p>
            </div>

            <div className="bg-[#f5f3f0] border-2 border-[#0066ff] rounded-lg p-4 mb-6">
              <h3 className="font-black text-[#0066ff] mb-2">📅 Payment Cycle</h3>
              <p className="text-lg font-black text-[#0066ff] mb-1">{selectedEqub.paymentCycle.duration} Months</p>
            ...
            <motion.button
              ...
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="w-full py-3 bg-[#0066ff] text-white font-black rounded-full hover:shadow-lg transition-all mb-3"
            >
              ✍️ Join This Equb
            </motion.button>
