...
export default function EqubCard({ equb }: EqubCardProps) {
  ...
  return (
    <div className="bg-card rounded-card border border-gray-200 p-4">
      ...
      <div className="mt-3 flex items-end justify-between">
        <div>
          ...
          <p className="text-base font-black text-[#0066ff]">{formatETB(equb.contribution_amount)}</p>
        </div>
      ...
      <div className="mt-3 flex items-center gap-2">
        <Link
          href={`/equbs/${equb.id}`}
          className="flex-1 text-center px-3 py-2 rounded-lg border border-gray-200 text-xs font-bold text-[#0066ff] hover:bg-gray-50 transition-colors"
        >
          View Equb
        </Link>
        <Link
