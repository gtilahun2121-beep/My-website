...
export default function EqubCard({ equb }: EqubCardProps) {
  ...
  return (
    <div className="bg-card rounded-card border border-slate-200 p-5">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="font-black text-slate-900 truncate">{equb.name}</p>
      ...
      <div className="mt-4">
        <div className="flex items-center justify-between text-xs font-semibold">
          <span className="text-slate-500">Round {equb.current_round} of {equb.total_rounds}</span>
      ...
      <div className="mt-4 flex items-end justify-between">
        <div>
          <p className="text-xs font-semibold text-slate-400">Contribution</p>
          <p className="text-lg font-black text-slate-900">{formatETB(equb.contribution_amount)}</p>
        </div>
        <div className="text-right">
          <p className="text-xs font-semibold text-slate-400">Payout pool</p>
          <p className="text-base font-bold text-slate-700">{formatETB(equb.total_amount)}</p>
        </div>
      </div>
      ...
      <div className="mt-4 flex items-center gap-2">
        <Link
          href={`/equbs/${equb.id}`}
          className="flex-1 text-center px-3 py-2 rounded-lg border border-slate-200 text-sm font-bold text-slate-700 hover:bg-slate-50 transition-colors"
        >
          View Equb
        </Link>
        <Link
